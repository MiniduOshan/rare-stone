    ...
    <div class="relative z-10 max-w-md w-full bg-surface border border-borderGray p-8 md:p-12 rounded-3xl shadow-2xl transition-all font-sans text-left">
        ...
        <form action="<?= BASE_URL; ?>/register/" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <div class="relative">
                    <input type="password" id="register-password" name="password" required placeholder="Minimally 6 characters" minlength="6" class="w-full bg-dark border border-gray-800 rounded-xl pl-4 pr-12 py-3 text-white text-sm focus:outline-none focus:border-gray-500 font-light transition-colors">
                    <button type="button" onclick="togglePasswordVisibility('register-password', 'eye-icon-register')" class="absolute inset-y-0 right-0 flex items-center pr-4 text-gray-500 hover:text-white transition-colors">
                        <i data-lucide="eye" id="eye-icon-register" class="w-4 h-4"></i>
                    </button>
                </div>
            </div>
