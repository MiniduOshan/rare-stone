...
<section class="pt-28 pb-6 px-8 md:px-16 max-w-7xl mx-auto relative z-20 border-b border-borderGray mb-8">
    <div class="flex flex-col lg:flex-row lg:items-end justify-between gap-6 pb-2">
        ...
        <div class="flex flex-wrap gap-2 lg:gap-3 text-[11px] uppercase tracking-[0.2em] font-medium">
            <button onclick="filterGems('all', this)" class="gem-filter-btn px-5 py-2 rounded-full border border-white text-white bg-white/10 transition-all">All Vault</button>
            <button onclick="filterGems('sapphire', this)" class="gem-filter-btn px-5 py-2 rounded-full border border-gray-800 text-gray-400 hover:border-gray-500 hover:text-white transition-all">Sapphires</button>
            <button onclick="filterGems('ruby', this)" class="gem-filter-btn px-5 py-2 rounded-full border border-gray-800 text-gray-400 hover:border-gray-500 hover:text-white transition-all">Rubies</button>
            <button onclick="filterGems('emerald', this)" class="gem-filter-btn px-5 py-2 rounded-full border border-gray-800 text-gray-400 hover:border-gray-500 hover:text-white transition-all">Emeralds</button>
            <button onclick="filterGems('diamond', this)" class="gem-filter-btn px-5 py-2 rounded-full border border-gray-800 text-gray-400 hover:border-gray-500 hover:text-white transition-all">Diamonds</button>
        </div>
    </div>
</section>
...
<section class="pb-32 px-8 md:px-16 max-w-7xl mx-auto relative z-20">
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-10" id="gemstones-grid">
        <?php foreach ($allGemstones as $gem): ?>
            <?php
                // Determine category tag for filtering
                $cat = 'sapphire';
                if (stripos($gem['title'], 'ruby') !== false) $cat = 'ruby';
